...
def get_restaurant(restaurant_id: int, db: Session = Depends(get_db)):
    ...
    # checks and looks up if restaurant even exists
    restaurant = db.query(Restaurant).filter(Restaurant.id == restaurant_id).first()

    # if restaurant doesnt exist then it will display the following
    if not restaurant:
        raise HTTPException(status_code=404, detail="Restaurant ID not found")

    return {"restaurant_id": restaurant.id}

# checks if the menu item belongs to its allocated staurant from the db
...
def get_menu_item(restaurant_id: int, food_item: str, db: Session = Depends(get_db)):
    ...
    # before we check if menu item belongs to restaurant, first checks if restaurant even exists
    rest = db.query(Restaurant).filter(Restaurant.id == restaurant_id).first()
    if rest is None:
        raise HTTPException(status_code=404, detail="Restaurant ID not found")

    # after that check if the menu item is available at its allocated restaurant
